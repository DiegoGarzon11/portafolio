...
window.addEventListener('load', () => {
	...
	function mensajes(mensaje, response) {
		...
		if (mensajesUser.length <= 1) {
		...
		} else if (mensaje.includes('thank') || mensaje.includes('gracias') || mensaje.includes('agradecido')) {
			textoChat.innerText = "No problem , you're welcome";
		} else if (mensaje.includes('how are you') || mensaje.includes('como esta') || mensaje.includes('como vas')) {
			textoChat.innerText = " I'm great, can i help you in something?";
		} else if (mensaje.includes('age') || mensaje.includes('años tiene diego') || mensaje.includes('edad') || mensaje.includes('old is diego')) {
			textoChat.innerText = `Sure, the answer to your question: ${inputChat.value} \n \n Diego is 21`;
		} else if (mensaje.includes('hobbies') || mensaje.includes('pasatiempos') || mensaje.includes('le gusta hacer a diego')) {
		...
		} else if (mensaje.includes('dedica diego') || mensaje.includes('What does Diego ?') || mensaje.includes('que hace diego')) {
			...
			textoChat.innerText = "Sorry, I don't have the answer right now, i hope help you as soon as possible :) ";
		}
